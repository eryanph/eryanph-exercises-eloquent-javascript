/**
 * https://eloquentjavascript.net/07_robot.html#i_s+ntyh5xrm
 * 
 * Persistent group
 ...
 *
 * Why do you need only one PGroup.empty value, rather than having a function that creates a new, empty map every
 * time?
 */
...
class PGroup {
  ...
  delete(value) {
    const newMembers = [...this.members];
    if (this.PHas(newMembers, value)) {
      newMembers.splice(newMembers.indexOf(value), 1);
    }
